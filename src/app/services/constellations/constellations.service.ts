...
export class ConstellationsService {
  ...
  private readonly API_URL = 'https://nexus-astralis.up.railway.app/api/Constellations'
  ...
  async getCommentsById(id: number): Promise<Comments[]> {
    const data = await fetch(`https://nexus-astralis.up.railway.app/api/Account/GetComments/${id}`);
